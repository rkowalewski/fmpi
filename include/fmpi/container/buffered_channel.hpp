...
class buffered_channel {
 ...
 private:
  ...
  channel_op_status try_push_(ValueType&& value) {
    ...
    ::new (static_cast<void*>(std::addressof(s->storage)))
        value_type(std::forward<ValueType>(value));
  ...
  channel_op_status try_value_pop_(slot*& s, std::size_t& idx) {
    ...
    // incrementing the slot cycle must be deferred till the value has been
    // consumed slot cycle tells procuders that the cell can be re-used (store
    // new value)
    return channel_op_status::success;
  }
