...
int Parent(int pnr, int p) {
  ...
  // If naive parent does not exist, we have to go to the parent of the
  // parent.
  if (parent_cnt > p) {
...
int RightChild(int pnr, int p) {
  ...
  // Right child does not exist always. It may happen that this child is at
  // the wrong place.
  if (i == 0) {
    // We are a leaf
    rchild = 0;
...
void Create(
    int myrank, int nprocs, int* lchild_top, int* rchild_top, int* parent) {
  // Calcuate children
