...
if (argv$output == '') {
    argv$output = paste0(remove_extension(basename(argv$input)),".bw")
}
...
p <- ggplot(data, aes_string(x=paste0("factor(", x_variable, ")"), y="mb", group="bench"))
p <- p + geom_line(position=pd, aes(linetype=bench, colour=bench)) +
    ...
    # scale_colour_brewer() +
    scale_color_npg() +
    ylab("Bandwidth (MB/s)") + xlab("Message Size (bytes)")
