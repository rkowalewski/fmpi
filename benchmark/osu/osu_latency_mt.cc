...
#define MYDBG(msg) \
  FMPI_DBG_STREAM("thread " << thread_id->id << ": " << msg)
  ...
  MYDBG("preparing dispatcher");
...
#if 0
    {
      ...
      for (i = val; i < options.iterations; i += options.sender_threads) {
        ...
        MYDBG(std::make_tuple(thread_id->id, hdl.id(), s_tag, r_tag));
        // MPI_CHECK(MPI_Send(s_buf, size, MPI_CHAR, 1, i, world.mpiComm()));
        // MPI_CHECK(MPI_Recv(
        //    r_buf, size, MPI_CHAR, 1, i, world.mpiComm(), &reqstat[val]));
        auto send = fmpi::make_send(
    ...
    if (flag_print == 1) {
      ...
      latency = (t)*1.0e6 / (2.0 * options.iterations / options.sender_threads);
      fprintf(
          stdout,
          "%-*d%*.*f\n",
          10,
